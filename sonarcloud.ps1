...
dotnet restore src
dotnet build Avans-Devops/Avans-Devops.sln --configuration release
dotnet test Avans-Devops --collect:"XPlat Code Coverage" --results-directory TestResults/ --logger "trx;LogFileName=unittests.trx" --no-build --no-restore --configuration release -- DataCollectionRunSettings.DataCollectors.DataCollector.Configuration.Format=opencover
